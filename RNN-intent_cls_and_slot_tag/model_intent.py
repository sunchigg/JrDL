...
class SeqClassifier(torch.nn.Module):
    def __init__(
        ...
        self.layer_1 = nn.Linear(hidden_size * 2, hidden_size)  # 分類工作適合最後接linear
        self.out = nn.Linear(hidden_size, num_class)
        self.act_fn = nn.ELU(0.1)  # ReLU, ELU, LeakyReLU(0.1)
    ...
    def forward(self, batch, h) -> Dict[str, torch.Tensor]:
        ...
        out = self.act_fn(out)
        out = self.dropout(out[:, -1, :].view(out.size(0), -1))  # reshape
