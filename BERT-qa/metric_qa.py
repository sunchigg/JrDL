...
class EM_F1(datasets.Metric):
    ...
    def _compute(self, predictions, references):
        ...
        ref_dict = {reference["id"]: {"answer": reference["answer"]["text"]} for reference in references}
        tokenizer = Tokenizer()
...
def compute_metrics(answer, predictions, tokenizer):
    metrics = []
    for id_ in tqdm(list(answer.keys()), desc='[*] Evaluating', dynamic_ncols=True):
        if id_ not in predictions:
            print(f'[!] Cannot find answer for id {id_} in model predictions')
            continue
        prediction = predictions[id_]
        metric = compute_metric(answer[id_]['answer'], prediction, tokenizer)
        metrics.append(metric)
        #if metric["em"] < 1 or metric["f1"] < 1:
            #print(id_, answer[id_]['answer'], prediction, metric["em"], metric["f1"])
